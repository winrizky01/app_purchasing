...
@section('content')
            ...
            <div class="card-body">
                <div class="row pb-2 gap-3 gap-md-0">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" name="filterType" id="filterType" placeholder="Enter Type">
                    </div>
                    <div class="col-md-4">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Create General</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" id="formGeneral">
                    @csrf
    ...
    <script type="text/javascript">
        // Setup Datatable
        var ajaxUrl  = "{{ url('setting/general/dataTables') }}";
        var ajaxData = [];
        var columns  = [{ data: 'name' }, { data: 'type' }, { data: 'description' }, { data: 'status' }, { data: 'action' }];
        var columnDefs  =  [
            {
                // User Status
                targets: -2,
        ...
        var buttons     =  [
            {
                ...
                action: function() {
                    $('#formGeneral')[0].reset();
                    $('#modalTitle').text('Create General');
                    $('#modalCreateEdit form').attr('action', '{{ url("setting/general/store") }}');
        ...
        $(document).ready(function() {
            ...
            $('.datatables tbody').on('click', '.edit-record', function() {
                $('#modalTitle').text('Edit General');
                $('#modalCreateEdit form').attr('action', '{{ url("setting/general/update") }}'+ '/' + $(this).data('id'));
            ...
            $('.datatables tbody').on('click', '.delete-record', function() {
                ...
                $('#confirmDelete').attr('action', '{{ url('setting/general/delete') }}/' + rowData.id);
            ...
            $('#handleFilter').click(function(){
                ...
                ajaxData = {'name':name, 'type':type, 'status': status};
                initializeDataTable(ajaxUrl, ajaxData, columns, columnDefs, buttons);
            });
        });
