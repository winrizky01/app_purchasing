...
@section('content')
            ...
            <div class="card-body">
                <div class="row pb-2 gap-3 gap-md-0">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" name="filterContactPerson" id="filterContactPerson" placeholder="Enter Contact Person">
                    </div>
                    <div class="col-md-4">
    ...
    <script type="text/javascript">
        // Setup Datatable
        var ajaxUrl  = "{{ url('master/vendor/dataTables') }}";
        var ajaxData = [];
        var columns  = [{ data: 'code' }, { data: 'name' }, {data: 'email'}, {data: 'contact_person'}, { data: 'status' }, { data: 'action' }];
        var columnDefs  =  [
            {
                // User Status
                targets: -2,
                ...
                render: function(data, type, full, meta) {
                    return (
                        '<div class="d-flex align-items-center">' +
                            '<a href="{{ url("master/vendor/edit") }}/' + full.id +
        ...
        var buttons     =  [
            {
                ...
                text: 'Add Vendor',
                action: function() {
                    window.location.href = '{{url("master/vendor/create")}}'; // Ganti URL_ANDA_DISINI dengan URL yang diinginkan
                }
            }, 
        ]
        // Setup Datatable
        ...
        $(document).ready(function() {
            ...
            $('.datatables tbody').on('click', '.delete-record', function() {
                ...
                $('#confirmDelete').attr('action', '{{url("master/vendor/delete")}}/'+rowData.id);
            ...
            $('#handleFilter').click(function(){
                ...
                if($('#filterContactPerson').val() != ""){
                    cp = $('#filterContactPerson').val();
                }
                if($('#filterstatus').val() != ""){
                ...
                ajaxData = {'name':name, 'contact_person':cp, 'status': status};
                initializeDataTable(ajaxUrl, ajaxData, columns, columnDefs, buttons);
            });
